...
function generateCaptcha1() {
    if(grecaptcha.getResponse() == "") {
      $('.recaptcha-err').text('Please Fill Re-captcha');
    }
}
    // Validate registration form
    function wpcrlValidateAndProcessRegisterForm() {
        ...
        $('#wpcrlRegisterForm').formValidation({
            ...
            fields: {
                ...
                wpcrl_lname: {
                    validators: {
                        ...
                        stringLength: {
                            max: 30,
                            message: 'The last must be less than 30 characters long'
                        },
                        regexp: {
                            regexp: /^[a-zA-Z]*$/,
                            message: 'Only characters are allowed.'
                ...
                wpcrl_county: {
                    message: 'The county is not valid',
                    validators: {
                        notEmpty: {
                            message: 'The county is required(Please select state first.)'
                        }
                    }
                },
                wpcrl_state: {
          ...
          if(grecaptcha.getResponse() == "") {
            e.preventDefault();
            $('.recaptcha-err').text('Please Fill Re-captcha');
            $('.reg-btn').removeClass('disabled').removeAttr('disabled');
          }else{
            $('.recaptcha-err').text('');
        ...
        var registerRequest = $.ajax({
            ...
            success: function(data) {
                ...
                if (true == data.reg_status) {
                    ...
                    $('#wpcrlRegisterForm').trigger("reset");
                    grecaptcha.reset();
